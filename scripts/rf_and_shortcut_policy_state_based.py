# -*- coding: utf-8 -*-
"""rf_and_shortcut_policy_state_based.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1H2pOCH0Y0H4hip5LuefQIuAlZW8nKL6N

## **History:**
- (29/10) Huge shoutout to Ryan (Ge) *Yan* for spotting two major bugs in the implementation of the Shortcut Policy.
- (19/10) This notebook was released
"""
...
class ConditionalUnet1D(nn.Module):
    def __init__(self,
        ...
        diffusion_distance_encoder = nn.Sequential(
            SinusoidalPosEmb(dsed),
            nn.Linear(dsed, dsed * 4),
            nn.Mish(),
            nn.Linear(dsed * 4, dsed),
        )
        cond_dim = 2 * dsed + global_cond_dim
        ...
        self.diffusion_distance_encoder = diffusion_distance_encoder
    ...
    def forward(self,
        ...
        global_time_feature = self.diffusion_step_encoder(timesteps)
        global_distance_feature = self.diffusion_distance_encoder(distance)

        global_feature = torch.cat([
            global_time_feature, global_distance_feature
        ], axis=-1)
...
#SOME CHANGES MADE BY ALPER
#- Removed EMA and LR schedule for simplicity
...
class RectifiedFlow():
  ...
  def get_train_tuple(self, z0=None, z1=None):
    t = torch.rand((z1.shape[0], ), device=z0.device)
    z_t =  t * z1 + (1.-t) * z0
    target = z1 - z0

    return z_t, t, target, None #to match API with Shortcut
  ...
  def sample_ode(self, z0=None, N=None, **model_kwargs):
    ...
    for i in range(N):
      ...
      pred = self.model(z, t.squeeze(), **model_kwargs)
...
class ShortcutModel():
  ...
  def get_train_tuple(self, z0=None, z1=None):
    ...
    distance = torch.zeros((z1.shape[0],), device=z0.device)
    t = torch.rand((z1.shape[0], 1, 1), device=z0.device)
    z_t =  t * z1 + (1.-t) * z0
    target = z1 - z0
  ...
  def get_shortcut_train_tuple(self, z0=None, z1=None, **model_kwargs):
    ...
    log_distance = torch.randint(low=0, high=7, size=(z1.shape[0],))
    distance = torch.pow(2, -1 * log_distance).to(z0.device)
    t = torch.rand((z1.shape[0], 1, 1), device=z0.device)

    z_t =  t * z1 + (1.-t) * z0

    s_t = self.model(z_t, t.squeeze(), distance=distance, **model_kwargs) #first small step
    z_tpd = z_t + s_t * distance.view(-1, 1, 1)
    tpd = t.squeeze() + distance
    s_tpd = self.model(z_tpd, tpd, distance=distance, **model_kwargs) #second small step
  ...
  def sample_ode(self, z0=None, N=None, **model_kwargs):
    ...
    traj.append(z.detach().clone())
    for i in range(N):
      t = torch.ones((batchsize,1,1), device=self.device) * i / N
      pred = self.model(z, t.squeeze(), **model_kwargs)
      z = z.detach().clone() + pred * dt
  ...
  @torch.no_grad()
  def sample_ode_shortcut(self, z0=None, N=None, **model_kwargs):
    ### NOTE: Use Euler method to sample from the learned flow
    if N is None:
      N = self.N
    dt = 1./N
...
rectified_flow = ShortcutModel(model=noise_pred_net, num_steps=100)
    ...
    for epoch_idx in tglobal:
        ...
        with tqdm(dataloader, desc='Batch', leave=False) as tepoch:
            for nbatch in tepoch:
                ...
                z_t, t, target, distance = rectified_flow.get_train_tuple(z0=noise, z1=naction)

                pred = rectified_flow.model(z_t, t, distance=distance, global_cond=obs_cond)
                ...
                #shortcut training step for equal batch size, in paper they use a quarter
                noise_shortcut = torch.randn(naction.shape, device=device)
                z_t, t, target, distance = rectified_flow.get_shortcut_train_tuple(z0=noise_shortcut, z1=naction, global_cond=obs_cond)
                pred_shortcut = rectified_flow.model(z_t, t, distance=distance, global_cond=obs_cond)
                ...
                tepoch.set_postfix(rf_loss=rf_loss_cpu, sc_loss=sc_loss_cpu)

        # Step the learning rate scheduler after each epoch
        lr_scheduler.step()

        tglobal.set_postfix(rf_loss=np.mean(rf_epoch_loss), sc_loss=np.mean(sc_epoch_loss), lr=optimizer.param_groups[0]['lr'])
...
with tqdm(total=max_steps, desc="Eval PushTStateEnv") as pbar:
    while not done:
        ...
        with torch.no_grad():
            ...
            ode_traj = rectified_flow.sample_ode_shortcut(z0=noisy_action, N=2, global_cond=obs_cond)
            naction = ode_traj[-1] #sample ode returns the whole traj, just take the last one
