...
class FlowMatchingScheduler:
    """
    Implementation of flow matching for generative modeling.
    Based on the paper: "Flow Matching for Generative Modeling"
    """
    def __init__(self, num_train_timesteps=100):
        """
        Initialize the flow matching scheduler.
    ...
    def set_timesteps(self, num_inference_timesteps=None):
        """
        Sets the discrete timesteps used for integration.
        
        Args:
            num_inference_timesteps: number of timesteps to use for inference
        """
        if num_inference_timesteps is None:
            num_inference_timesteps = self.num_train_timesteps
            
        self.timesteps = torch.linspace(0, 1, num_inference_timesteps)
        return self.timesteps

# Modified training script with checkpoint saving and loading
...
if __name__ == "__main__":
    ...
    action_horizon = 8
    num_epochs = 100
    num_flow_timesteps = 100
    save_checkpoint_interval = 20  # Save every 20 epochs
    resume_from_checkpoint = False  # Set to True to resume training
    checkpoint_path = 'checkpoints/flow_matching_latest.pt'  # Path to resume from
    ...
    # Create network object - same architecture as original
    ...
    # Initialize the flow matching scheduler
    flow_scheduler = FlowMatchingScheduler(num_train_timesteps=num_flow_timesteps)
    ...
    if resume_from_checkpoint and os.path.exists(checkpoint_path):
        def load_checkpoint(checkpoint_path, nets, optimizer=None, lr_scheduler=None, device='cuda'):
            ...
            # If optimizer is provided (resuming training)
            if optimizer is not None:
                optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
            
            # If lr_scheduler is provided (resuming training)
            if lr_scheduler is not None and 'lr_scheduler_state_dict' in checkpoint:
                lr_scheduler.load_state_dict(checkpoint['lr_scheduler_state_dict'])
            
            # Get the epoch and stats
            epoch = checkpoint.get('epoch', 0)
            loaded_stats = checkpoint.get('stats', None)
            
            print(f"Loaded checkpoint from epoch {epoch}")
            return epoch, loaded_stats, nets
            
        start_epoch, loaded_stats, nets = load_checkpoint(
            checkpoint_path, nets, optimizer, lr_scheduler, device
        )
        
        # Use loaded stats if available
        ...
        for epoch_idx in tglobal:
            ...
            with tqdm(dataloader, desc='Batch', leave=False) as tepoch:
                for nbatch in tepoch:
                    ...
                    x0 = torch.randn_like(naction, device=device)
                    # Target actions as ending point (x1)
                    x1 = naction
                    
                    # Sample random timesteps between 0 and 1
                    t = torch.rand(B, device=device)
                    
                    # Get interpolated point at time t
                    # x_t = (1-t) * x0 + t * x1
                    t_reshape = t.reshape(-1, 1, 1)  # [B, 1, 1]
                    x_t = (1 - t_reshape) * x0 + t_reshape * x1
                    
                    # Get true velocity field
                    # For linear flow, velocity is constant at all t: v_t = x1 - x0
                    v_t = x1 - x0
                    
                    # Predict velocity using the UNet model
                    # The same UNet architecture is used, but now it predicts the flow velocity
                    # instead of the noise residual
                    v_pred = nets['noise_pred_net'](
                        sample=x_t,
                        timestep=t,
                        global_cond=obs_cond
                    )
                    
                    # L2 loss between predicted and true velocity
                    loss = nn.functional.mse_loss(v_pred, v_t)
            ...
            if ((epoch_idx + 1) % save_checkpoint_interval == 0) or (epoch_idx == num_epochs - 1):
                ...
                checkpoint = {
                    ...
                    'stats': stats  # Save the dataset statistics for inference
                }
                
                # Save checkpoint with epoch number
                checkpoint_path = os.path.join('checkpoints', f'flow_matching_epoch_{epoch_idx+1}.pt')
                torch.save(checkpoint, checkpoint_path)
                print(f"Checkpoint saved: {checkpoint_path}")
                
                # Also save the latest model (overwrite)
                latest_path = os.path.join('checkpoints', 'flow_matching_latest.pt')
    ...
    with tqdm(total=max_steps, desc="Eval PushTImageEnv") as pbar:
        while not done:
            ...
            # infer action with flow matching
            with torch.no_grad():
                # get image features
                ...
                for i, t in enumerate(flow_scheduler.timesteps):
                    ...
                    if i < len(flow_scheduler.timesteps) - 1:
                        # dt = t_{i+1} - t_i
                        dt = flow_scheduler.timesteps[i + 1] - t
                        # x_{t+dt} = x_t + v_t * dt
                        x_t = x_t + dt * v_t
    ...
    # visualize
    vwrite('vis_flow_matching.mp4', imgs)
    Video('vis_flow_matching.mp4', embed=True, width=256, height=256)