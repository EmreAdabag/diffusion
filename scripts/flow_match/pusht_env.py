...
Standalone implementation of the Push-T environment.
"""
...
class DrawOptions(pymunk.SpaceDebugDrawOptions):
    """Custom drawing options for pymunk space."""
...
def pymunk_to_shapely(body, shapes):
    """Convert pymunk shapes to shapely geometries."""
    geoms = list()
    for shape in shapes:
        if isinstance(shape, pymunk.shapes.Poly):
            ...
            geoms.append(sg.LineString([a, b]).buffer(shape.radius))
...
class PushTImageEnv(PushTEnv):
    """
    Vision-based version of PushTEnv that returns image observations
    """
    def __init__(self, render_size=96, **kwargs):
        super().__init__(render_size=render_size, **kwargs)
        
        # Observation space is now a dict with image and agent position
        self.observation_space = spaces.Dict({
            ...
            'agent_pos': spaces.Box(
                ...
                high=np.array([self.window_size, self.window_size], dtype=np.float32),
                shape=(2,),
                dtype=np.float32
            )
        })
